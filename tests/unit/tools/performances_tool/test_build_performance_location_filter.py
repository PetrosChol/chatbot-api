...
from app.tools.statement_builders.performances_statement_builder import (
    _build_performance_location_filter,
)
...
def test_build_performance_location_filter_default_wildcard(mock_or, mock_build_fuzzy):
    """Test that the default ['*'] input results in no filter (returns None)."""
    result = _build_performance_location_filter(["*"])
...
def test_build_performance_location_filter_empty_list(mock_or, mock_build_fuzzy):
...
@patch("app.tools.statement_builders.performances_statement_builder.or_")
def test_build_performance_location_filter_only_wildcards_or_empty_strings(
    mock_or, mock_build_fuzzy
):
    """Test lists containing only '*' or empty strings, assuming build_fuzzy handles them."""
    locations = ["*", "", "   ", "*"]
    result = _build_performance_location_filter(locations)
    assert result is None
...
@patch("app.tools.statement_builders.performances_statement_builder.or_")
def test_build_performance_location_filter_single_location(mock_or, mock_build_fuzzy):
    """Test with a single valid location string using fuzzy matching."""
    mock_condition = MockSQLAlchemyFuzzyCondition("fuzzy_for_location1")
...
def test_build_performance_location_filter_multiple_locations(
    mock_or, mock_build_fuzzy
):
    """Test with multiple valid location strings using fuzzy matching, expects or_()."""
    mock_condition1 = MockSQLAlchemyFuzzyCondition("fuzzy_for_location1")
    mock_condition2 = MockSQLAlchemyFuzzyCondition("fuzzy_for_location2")
...
@patch("app.tools.statement_builders.performances_statement_builder.or_")
def test_build_performance_location_filter_mixed_locations(mock_or, mock_build_fuzzy):
    """Test with mixed valid, wildcard, and empty locations using fuzzy matching."""
    mock_condition1 = MockSQLAlchemyFuzzyCondition("fuzzy_for_location1")
    mock_condition3 = MockSQLAlchemyFuzzyCondition("fuzzy_for_location3")
...
@patch("app.tools.statement_builders.performances_statement_builder.or_")
def test_build_performance_location_filter_no_conditions_returned(
    mock_or, mock_build_fuzzy
):
    """Test when build_fuzzy_conditions returns an empty list."""
    locations = ["NonMatchingLocation"]
    result = _build_performance_location_filter(locations)
